...
export default function VideoCallRoomPage() {
  ...
  const call = useVideoCall({
    userId: user?.doctorProfileId || user?.id || 0,
    userType: 'doctor',
  ...
  const handleEndCall = () => {
    call.endCall();
    setTimeout(() => router.push('/doctor/video-sessions'), 1000);
  };
            ...
            <div className="text-center">
              {call.callState === 'idle' && (
                <>
                  ...
                  <p className="text-white text-lg mb-6">Ready to start video consultation</p>
                  <button
                    onClick={() => call.startCall(0, 'patient')}
              ...
              {call.callState === 'calling' && (
                <>
                  ...
                  <p className="text-white/50 text-sm mt-2">Waiting for patient to connect</p>
                </>
              )}
      ...
      <div className="flex items-center justify-center gap-4 py-6 bg-black/50 backdrop-blur-sm">
        <button
          ...
          className={`w-14 h-14 rounded-full flex items-center justify-center transition-colors ${
            call.micOn ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-red-600 text-white'
          }`}
        >
        ...
        <button
          ...
          className={`w-14 h-14 rounded-full flex items-center justify-center transition-colors ${
            call.cameraOn ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-red-600 text-white'
          }`}
        >
        ...
        <button
          ...
          className="w-16 h-16 rounded-full bg-red-600 text-white flex items-center justify-center hover:bg-red-500 transition-colors shadow-lg shadow-red-600/30"
        >
