...
interface Conversation {
  patient?: { id: number; fullName: string; ethioChartId: string; email: string };
...
export default function DoctorMessagesPage() {
  ...
  const doctorProfileId = user?.doctorProfileId || 0;

  // Determine chat params based on selected conversation
  const patientId = selectedConv?.patient?.id || 0;
  const chat = useChat({
  ...
  const loadConversations = async () => {
    try {
      const res = await api.get(`/chat/conversations/doctor/${doctorProfileId}`);
  ...
  useEffect(() => {
    if (!selectedConv?.patient?.id || !doctorProfileId) return;
    loadThread();
  }, [selectedConv?.patient?.id, doctorProfileId]);

  const loadThread = async () => {
    try {
      const res = await api.get(`/chat/thread/${selectedConv!.patient!.id}/${doctorProfileId}`);
  ...
  const filteredConvs = conversations.filter((c) =>
    ...
    c.patient?.ethioChartId?.toLowerCase().includes(search.toLowerCase())
  );
          ...
          <div className="p-4 border-b border-outline-variant/20">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 border border-outline-variant/30 text-sm focus:ring-0 focus:border-primary"
              />
            </div>
          </div>
              ...
              filteredConvs.map((c, i) => (
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors ${
                    selectedConv?.patient?.id === c.patient?.id
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between">
                      <p className="text-sm font-semibold text-on-surface truncate">{c.patient?.fullName || 'Patient'}</p>
            ...
            <>
              ...
              <div className="px-6 py-4 border-b border-outline-variant/20 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full bg-primary-container flex items-center justify-center text-white font-bold text-sm">
                      {selectedConv.patient?.fullName?.charAt(0) || 'P'}
                    </div>
                  ...
                  <div>
                    <p className="font-semibold text-sm">
                      {selectedConv.patient?.fullName}
                      <span className="font-mono text-[10px] bg-primary-container/10 text-primary-container px-2 py-0.5 ml-2">{selectedConv.patient?.ethioChartId}</span>
                    </p>
                    <p className={`text-[10px] ${chat.isRecipientOnline ? 'text-green-600' : 'text-on-surface-variant'}`}>
                      {chat.isTyping ? `${chat.typingUser} is typing...` : chat.isRecipientOnline ? '● Online' : '○ Offline'}
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <button className="text-outline hover:text-primary transition-colors"><span className="material-symbols-outlined">call</span></button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3 bg-surface-container-low/30">
                {chat.messages.length === 0 ? (
                  <p className="text-center text-sm text-on-surface-variant py-12">No messages yet — start the conversation</p>
                ) : (
                  chat.messages.map((m: any, i: number) => (
                    <div key={m.id || i} className={`flex gap-3 ${m.senderType === 'doctor' ? 'justify-end' : ''}`}>
                      {m.senderType === 'patient' && (
                        <div className="w-8 h-8 rounded-full bg-surface-container flex items-center justify-center text-xs flex-shrink-0">
                          {selectedConv.patient?.fullName?.charAt(0) || 'P'}
                        </div>
                      )}
                      <div className={`px-4 py-3 rounded-lg max-w-md ${
                        m.senderType === 'doctor' ? 'bg-primary-container text-white' : 'bg-white shadow-sm'
                      }`}>
                        <p className="text-sm">{m.content}</p>
                        <div className={`flex items-center gap-1 mt-1 ${m.senderType === 'doctor' ? 'text-white/70' : 'text-on-surface-variant'}`}>
                          <span className="text-[10px]">{formatTime(m.createdAt)}</span>
                          {m.senderType === 'doctor' && m.isRead && <span className="material-symbols-outlined" style={{ fontSize: 12 }}>done_all</span>}
                        </div>
                      </div>
                      {m.senderType === 'doctor' && (
                        <div className="w-8 h-8 rounded-full bg-primary-container flex items-center justify-center text-white text-xs flex-shrink-0 font-bold">D</div>
                      )}
                    </div>
                  ))
                )}
                {chat.isTyping && (
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-surface-container flex items-center justify-center text-xs">
                      {selectedConv.patient?.fullName?.charAt(0) || 'P'}
                    </div>
              ...
              <div className="px-6 py-4 border-t border-outline-variant/20">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="w-10 h-10 bg-primary-container text-white rounded flex items-center justify-center hover:bg-primary transition-colors disabled:opacity-50"
                  >
